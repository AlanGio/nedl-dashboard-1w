              ...
              <tr>
                ...
                {updatedPolicies.map((policy) => (
                  <th
                    key={policy.id}
                    className={cn(
                      "w-1/6 rounded-md p-3 text-center text-sm font-medium text-slate-700",
                      policy.bgColor,
                    )}
                  >
                    {policy.name}
              ...
              {services.map((service) => (
                <tr key={service.id}>
                  <td className="py-4 pr-4">
                    ...
                    <button className="rounded-md bg-purple-50 px-3 py-1 text-[10px] font-medium text-purple-600 hover:bg-purple-100">
                      COMPARE CRITERIA
                    </button>
                  </td>
                  {updatedPolicies.map((policy) => {
                    ...
                    return (
                      <td key={policy.id} className="py-4 text-center">
                        <div
                          className={cn(
                            "mx-auto inline-block rounded-md border px-4 py-1 text-center text-xs",
                            status.type === "covered"
